-- MySQL 远程部署初始化脚本（阿里云 CentOS 适用）
-- 作用：创建数据库、账号、表结构，并插入示例数据（Mock数据）
-- 使用方式（在服务器上执行）：
--   mysql -u root -p < /path/to/mysql-remote-init.sql

-- 1) 创建数据库
CREATE DATABASE IF NOT EXISTS saber_consulting
  CHARACTER SET utf8mb4
  COLLATE utf8mb4_unicode_ci;
USE saber_consulting;

-- 2)（可选）创建远程访问账号（按需修改强密码！）
-- 如已手动创建，可忽略此段
CREATE USER IF NOT EXISTS 'saber_user'@'%' IDENTIFIED BY 'saber_password';
GRANT ALL PRIVILEGES ON saber_consulting.* TO 'saber_user'@'%';
FLUSH PRIVILEGES;
...
CREATE TABLE IF NOT EXISTS articles (
  ...
  content MEDIUMTEXT NOT NULL COMMENT '中文内容',
  content_en MEDIUMTEXT NOT NULL COMMENT '英文内容',
...
CREATE TABLE IF NOT EXISTS inquiries (
  ...
  message MEDIUMTEXT NOT NULL COMMENT '咨询内容',
  status ENUM('new','processing','completed','archived') DEFAULT 'new' COMMENT '处理状态',
  admin_notes MEDIUMTEXT COMMENT '管理员备注',
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
...
CREATE TABLE IF NOT EXISTS system_settings (
  id INT AUTO_INCREMENT PRIMARY KEY,
  setting_key VARCHAR(100) NOT NULL UNIQUE COMMENT '键',
  setting_value MEDIUMTEXT COMMENT '值',
...
-- 4) 插入示例数据（Mock）
INSERT INTO articles (title, title_en, content, content_en, category, author)
VALUES
...
-- 5) 视图（可选示例）
DROP VIEW IF EXISTS article_summary;
CREATE VIEW article_summary AS
SELECT a.id, a.title, a.title_en, a.category, a.author, a.status, a.views,
       a.created_at,
...
-- 完成
SELECT 'Database initialized (schema + seed) for saber_consulting' AS msg;
